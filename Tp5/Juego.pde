class Juego {
  ...
  Persj p;
  Objetivo o;
  Diablo diablo; // instancia individual, no usada en el juego
  int vidas;
  int objetos;
  int time;
  boolean agarrarOb;
    ...
    //---------------------------------------------------------------
     if(estado.equals("portada")){
      ...
      text("Mini Juego",width/2, 150);
       ...
       text("Utliza las flechas para moverte.",width/2, 450);
  ...
  if(estado.equals("derrota")){
    background(0);
     ...
     textSize(100);
      text("Te acaban de \n atrapar",width/2, 150);
       textSize(30);
      text("Presiona R para volverlo a intentar",width/2, 450);
       if(key == 'R' || key == 'r'){
   if(estado =="derrota"){
     estado = "portada";
   }
